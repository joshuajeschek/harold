...
import { createFilename, getQuery } from '../lib/utils';
...
export class VoteRoute extends Route {
	...
	private imageChannel?: TextBasedChannel;
	...
	private async createVoteCard(template: string, yes: string, no: string, ctx: string, author?: string) {
		...
		this.imageChannel ||= (await this.container.client.channels.fetch('813551053844643860')) as TextBasedChannel;

		const filename = createFilename(template, 'png');
		...
		await image.writeAsync(filename).catch((e) => this.container.logger.error('Error while saving a vote image:', e));

		const url = (await this.imageChannel.send({ files: [{ attachment: filename, name: filename }] })).attachments.first()?.url;

		unlink(filename).catch((e) => this.container.logger.error('Error while deleting a vote image:', e));
